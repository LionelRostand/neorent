...
const Taxes = () => {
  ...
  // Add null check for fiscalities array
  const currentYear = new Date().getFullYear();
  const safeDeclarations = fiscalities || [];
  const totalDeclarations = safeDeclarations.length;
  const paidDeclarations = safeDeclarations.filter(d => d.status === 'Payée').length;
  const pendingDeclarations = safeDeclarations.filter(d => d.status === 'À déclarer').length;
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <span className={`px-3 py-1 rounded-full text-sm ${
                          declaration.status === 'Payée' ? 'bg-green-100 text-green-800' :
                          declaration.status === 'À déclarer' ? 'bg-yellow-100 text-yellow-800' :
