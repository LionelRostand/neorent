      ...
      case 'En attente':
        return (
          <Badge className={`bg-yellow-100 text-yellow-800 ${className}`}>
            ...
            {t('tenantHistory.status.pending')}
          </Badge>
        );
      case 'En retard':
        return (
          <Badge variant="destructive" className={className}>
            ⚠️ {t('tenantHistory.status.overdue')}
          </Badge>
        );
      default:
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg md:text-xl">
          ...
          {t('tenantHistory.rentHistory')} {contractData ? `- ${t('tenantHistory.since')} ${new Date(contractData.startDate).toLocaleDateString()}` : `- ${t('tenantHistory.since')} September 2023`}
        </CardTitle>
      </CardHeader>
      <CardContent>
              ...
              <div className="flex items-center space-x-3 md:space-x-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className={`flex ${isMobile ? 'flex-col space-y-1' : 'items-center gap-4'} text-xs md:text-sm text-gray-600 mt-1`}>
                    ...
                    {payment.paymentDate && (
                      <span>{t('tenantHistory.paidOn')}: {new Date(payment.paymentDate).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className={`flex ${isMobile ? 'flex-col space-y-3' : 'items-center space-x-4'}`}>
                <div className={`${isMobile ? 'flex justify-between items-center' : 'text-right'}`}>
                  <div>
                    ...
                    {payment.status === 'Payé' && (
                      <div className="mb-2">
                        <div className="flex items-center gap-1 justify-end">
                          ...
                          <span className="text-xs text-green-600 font-medium">{t('tenantHistory.amountPaid')}</span>
                        </div>
                    ...
                    {payment.status !== 'Payé' && (
                      <div className="mb-2">
                        <span className="text-xs text-gray-500">{t('tenantHistory.amountToPay')}</span>
                    ...
                    <p className="text-xs md:text-sm text-gray-600">
                      {t('tenantHistory.rent')}: {payment.rent}€ + {t('tenantHistory.charges')}: {payment.charges}€
                    </p>
                  </div>
                </div>
